...
lm.series.full <- sapply(2:ncol(df.predict.2010), function(x){
  ...
  lm.trial <- lm(d_vote_current ~ d_vote_prior + df.predict.2010[,x])
...
lm.series.notwitter <- sapply(2:ncol(df.predict.2010), function(x){
  ...
  lm.trial <- lm(d_vote_current ~ d_vote_prior)
  lm.predict <- predict(lm.trial)
...
lm.series.novote <- sapply(2:ncol(df.predict.2010), function(x){
  ...
  d_vote = 100 - df.results.2010$r_vote_2010
...
lm.full = lm(d.vote.current ~ d.vote.prior + twitter.forecast, data=input.df)
