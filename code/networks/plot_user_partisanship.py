...
pship = pd.read_csv('../../data/mnb_user_partisanship.csv')
...
bool_pship = [False if n in extraneous_names else True
              for n in pship.user
              ]

tweets = tweets[bool_tweets]
pship = pship[bool_pship]
...
## the partisan label
global_nodelist = []
pship.set_index('user', inplace=True)
for node in global_nodedict.keys():
    try:
        ncolor = pship.ix[node]['pship']
    except KeyError:
        ncolor = 0
...
pship.reset_index(inplace=True)
pship.set_index('user', inplace=True)
lib_nodes = [n for n in global_rt_graph.nodes() if n in pship.index and
             pship.ix[n]['pship_label'] in ['lib', 'very_lib']
             ]
con_nodes = [n for n in global_rt_graph.nodes() if n in pship.index and
             pship.ix[n]['pship_label'] in ['con', 'very_con']
             ]
...
with open('../../data/rt_path_lengths.csv', 'wt') as f:
    writer = csv.writer(f)
...
colormap = {0:'#000066', 1:'#660066', 2:'#990033', 3:'#FF0000'}
node_color = nx.get_node_attributes(largest_cc_mst, 'color')
colorvec = [colormap[node_color[n]] for n in node_color]
